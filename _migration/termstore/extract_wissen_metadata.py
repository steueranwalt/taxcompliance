#!/usr/bin/env python3
"""Extract Jahr/Autor/Werk/Seite/Fundstelle (+ heuristics) from Notizbuch markdown.
...
def process_file(path: Path, root: Path) -> dict:
    ...
    sp_rel = f"Wissen/{root.name}/{rel}"  # Shared Documents/Wissen/Notizbuch-fuer-Wissen/...
    return {
        "FileLeafRef": path.name,
        "ServerRelativePath": sp_rel.replace("\\", "/"),
...
def main() -> int:
    ...
    filled = sum(1 for r in rows if r["Jahr"] or r["Werk"] or r["Autor"] or r["Dokumenttyp"])
